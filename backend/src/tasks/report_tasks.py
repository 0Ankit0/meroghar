...
from ..models.report import ReportTemplate, GeneratedReport, ReportType
...
async def _generate_scheduled_reports_async() -> Dict:
    ...
    async with async_session_maker() as session:
        try:
            ...
            for template in templates:
                ...
                try:
                    ...
                    if not _should_generate_report(template, today):
                        logger.debug(
                            f"Skipping template {template.id} - not due today"
                        )
                        continue
            ...
            logger.error(
                f"Failed to generate scheduled reports: {str(e)}", 
                exc_info=True
            )
    ...
    if template.report_type in [ReportType.TAX_INCOME, ReportType.TAX_DEDUCTIONS]:
        ...
        if frequency == 'monthly':
            ...
            if current_quarter == 1:
                ...
                period_end = date(today.year, prev_quarter_start_month + 3, 1) - datetime.timedelta(days=1)
...
def generate_report_by_id(self, template_id: str, user_id: str, parameters: Optional[Dict] = None):
    ...
    try:
        with get_db_context() as db:
            ...
            logger.info(
                f"Generated report {generated_report.id} from template {template_id}"
            )
        ...
        logger.error(
            f"Failed to generate report from template {template_id}: {str(e)}",
            exc_info=True
        )
        return {
            "status": "failed",
...
async def _cleanup_old_reports_async(retention_days: int) -> Dict:
    ...
    async with async_session_maker() as session:
        try:
            ...
            logger.info(
                f"Found {len(old_reports)} reports older than {retention_days} days"
            )
            ...
            for report in old_reports:
                try:
                    ...
                    logger.error(
                        f"Failed to delete report {report.id}: {str(e)}",
                        exc_info=True
                    )
