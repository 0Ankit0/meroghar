...
def send_payment_confirmation(self, payment_id: str):
    ...
    result = loop.run_until_complete(
        _send_payment_confirmation_async(UUID(payment_id))
    )
    return result
    ...
    async with async_session_maker() as session:
        try:
            ...
            return {
                ...
                "message": f"Notifications sent via: {', '.join(notifications_sent)}" if notifications_sent else "No notifications sent",
...
async def _send_sms_notification(phone: str, data: Dict[str, any]) -> None:
    ...
    logger.info(
        f"[SMS PLACEHOLDER] Would send SMS to {phone}:\n{sms_body}"
    )
