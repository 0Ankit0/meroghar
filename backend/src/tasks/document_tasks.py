                ...
                for document in documents:
                    try:
                        ...
                        if not tenant or not tenant.user or not tenant.user.phone:
                            logger.warning(
                                f"Document {document.id} has no valid tenant contact info"
                            )
                            continue
                        ...
                        expiry_date = (
                            document.expiration_date.strftime("%Y-%m-%d")
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to send push notification for document {document.id}: {e}"
                            )
                        ...
                        sent_count += 1
                        logger.info(
                            f"Sent expiration reminder for document {document.id} to tenant {tenant.id}"
                        )

                    except Exception as e:
                        logger.error(
                            f"Failed to send reminder for document {document.id}: {str(e)}"
                        )
                        failed_count += 1

                logger.info(
                    f"Sent {sent_count} document expiration reminders, {failed_count} failed"
                )
            ...
            try:
                ...
                result = await session.execute(
                    select(Document).where(
                        Document.status == DocumentStatus.DELETED, Document.updated_at < cutoff_date
                    )
                )
