...
class SyncService:
    ...
    def resolve_conflict_lww(
        ...
        if client_updated_at > server_updated_at:
            return True, f"Client record is newer (client: {client_updated_at}, server: {server_updated_at})"
        elif client_updated_at < server_updated_at:
            return False, f"Server record is newer (server: {server_updated_at}, client: {client_updated_at})"
        else:
        ...
        try:
            if operation == 'create':
                ...
                if not record_id:
                    return {
                        'status': 'failed',
                        'message': f"Update operation requires 'id' field",
                    }
                ...
                should_update, reason = SyncService.resolve_conflict_lww(
                    server_record, record_data
                )
                ...
                if should_update:
                    ...
                    return {
                        ...
                        'conflict_details': {
                            ...
                            'server_data': {
                                'id': server_record.id,
                                'updated_at': server_record.updated_at.isoformat() if hasattr(server_record, 'updated_at') else None,
                            },
                            'reason': reason,
                        },
                    }
                ...
                if not record_id:
                    return {
                        'status': 'failed',
                        'message': f"Delete operation requires 'id' field",
                    }
