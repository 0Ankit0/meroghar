...
class ExportService:
    ...
    def export_payment_history_excel(
        ...
        for row_num, payment in enumerate(payments, data_start_row):
            ws.cell(row=row_num, column=1).value = payment.payment_date.strftime('%Y-%m-%d') if payment.payment_date else ''
            ws.cell(row=row_num, column=2).value = float(payment.amount)
            ws.cell(row=row_num, column=3).value = payment.payment_method.value if payment.payment_method else ''
            ...
            ws.cell(row=row_num, column=6).value = float(payment.gateway_fee) if payment.gateway_fee else 0
    ...
    def export_payment_history_pdf(
        ...
        if start_date or end_date:
            date_range = f"<br/><b>Period:</b> {start_date or 'Beginning'} to {end_date or 'Present'}"
        ...
        # Create table
        table = Table(table_data, colWidths=[1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1.8*inch])
