...
class NotificationService:
    ...
    @classmethod
    def initialize_fcm(cls, credentials_path: Optional[str] = None) -> None:
    ...
    async def create_notification(
        ...
        if send_push:
            try:
                ...
                logger.error(
                    f"Failed to send push notification {notification.id}: {e}"
                )
    ...
    async def send_push_notification(
        ...
        result = await db.execute(
            select(User.fcm_token).where(User.id == notification.user_id)
        )
        ...
        try:
            ...
            logger.info(
                f"Push notification {notification.id} sent successfully: "
                f"{fcm_message_id}"
            )
            ...
            logger.error(
                f"Failed to send FCM message for notification {notification.id}: {e}"
            )
            notification.delivery_failed = True
            notification.failure_reason = str(e)
            await db.commit()
