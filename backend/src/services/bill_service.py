...
from ..models.bill import (AllocationMethod, Bill, BillAllocation, BillStatus,
                           RecurringBill, RecurringFrequency)
...
class BillService:
    ...
    async def create_bill(
        ...
        created_by: UUID,
        allocations: list[BillAllocationCreateRequest] | None = None,
    ) -> Bill:
        ...
        result = await self.session.execute(
            select(Bill).options(selectinload(Bill.allocations)).where(Bill.id == bill.id)
        )
    ...
    async def _create_automatic_allocations(
        ...
        custom_percentages: list[BillAllocationCreateRequest] | None = None,
    ) -> None:
        ...
        if method == AllocationMethod.EQUAL:
            ...
            percentage_map = {alloc.tenant_id: alloc.percentage for alloc in custom_percentages}

            # Validate all tenants have percentages
            for tenant in active_tenants:
                if tenant.id not in percentage_map:
                    raise ValueError(f"Missing percentage for tenant {tenant.id}")
    ...
    async def update_bill_status(
        ...
        """
        result = await self.session.execute(
            select(Bill).options(selectinload(Bill.allocations)).where(Bill.id == bill_id)
        )
    ...
    async def mark_allocation_paid(
        ...
        logger.info(
            f"Allocation marked paid: allocation_id={allocation_id}, " f"payment_id={payment_id}"
        )
        ...
        for recurring in recurring_bills:
            ...
            if recurring.frequency == RecurringFrequency.MONTHLY:
                ...
                if period_start.month == 12:
                    ...
                    period_end = date(period_start.year, period_start.month + 1, 1) - timedelta(
                        days=1
                    )
            ...
            try:
                ...
                logger.error(f"Failed to generate bill from recurring {recurring.id}: {str(e)}")
