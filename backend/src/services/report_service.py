...
from ..models.report import (GeneratedReport, ReportFormat, ReportTemplate,
                             ReportType)
...
class ReportService:
    ...
    def calculate_annual_income(
        ...
        for payment in payments:
            ...
            payment_method_breakdown[method] = (
                payment_method_breakdown.get(method, Decimal("0.00")) + amount
            )
        ...
        return {
            ...
            "income_by_property": {
                k: {
                    **v,
                    "total_income": float(v["total_income"]),
    ...
    def calculate_deductible_expenses(
        ...
        return {
            ...
            "by_property": {
                k: {
                    **v,
                    "total_expenses": float(v["total_expenses"]),
    ...
    def calculate_gst_report(
        ...
        self.calculate_annual_income(user_id, year, property_ids)
    ...
    def generate_profit_loss_statement(
        ...
        # Categorize revenue
        rental_income = sum(p.amount for p in payments if p.payment_type == PaymentType.RENT)
        utility_income = sum(p.amount for p in payments if p.payment_type == PaymentType.UTILITY)
        other_income = sum(
            p.amount
            for p in payments
            if p.payment_type
            not in [PaymentType.RENT, PaymentType.UTILITY, PaymentType.SECURITY_DEPOSIT]
        )
        ...
        for expense in expenses:
            ...
            expense_by_category[category] = (
                expense_by_category.get(category, Decimal("0.00")) + expense.amount
            )
    ...
    def generate_report(
        ...
        if template.report_type == ReportType.TAX_INCOME:
            ...
            self.calculate_annual_income(generated_by, year)
        elif template.report_type == ReportType.TAX_DEDUCTIONS:
            year = params.get("year", datetime.now().year)
            self.calculate_deductible_expenses(generated_by, year)
        elif template.report_type == ReportType.TAX_GST:
            year = params.get("year", datetime.now().year)
            quarter = params.get("quarter", 1)
            self.calculate_gst_report(generated_by, quarter, year)
        elif template.report_type == ReportType.PROFIT_LOSS:
            self.generate_profit_loss_statement(generated_by, period_start, period_end)
        elif template.report_type == ReportType.CASH_FLOW:
            self.generate_cash_flow_report(generated_by, period_start, period_end)
        else:
    ...
    def verify_share_token(self, token: str) -> GeneratedReport | None:
        ...
        """
        report = self.db.query(GeneratedReport).filter(GeneratedReport.share_token == token).first()
