...
from ...schemas.sync import (ConflictResponse, SyncRecordResponse, SyncRequest,
...
async def bulk_sync(
    ...
    x_device_name: Annotated[
        str | None, Header(None, description="Human-readable device name")
    ] = None,
):
    """
    ...
    if not sync_request.records:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No records provided for sync"
        )
    ...
    try:
        ...
        await SyncService.update_sync_log_status(
            ...
            conflict_details=(
                {"conflicts": conflict_details_list} if conflict_details_list else None
            ),
        )
...
async def get_sync_status(
    current_user: Annotated[User, Depends(get_current_user)],
    session: Annotated[AsyncSession, Depends(get_db)],
    x_device_id: Annotated[str | None, Header(None, description="Filter by device ID")] = None,
):
    """
