...
async def khalti_callback(
    ...
    txnId: str | None = Query(None, description="Transaction ID from Khalti"),
    amount: int | None = Query(None, description="Amount in paisa"),
    mobile: str | None = Query(None, description="Payer mobile number"),
    purchase_order_name: str | None = Query(None, description="Purchase order name"),
    transaction_id: str | None = Query(None, description="Transaction ID"),
) -> dict:
    """
    ...
    try:
        ...
        if len(parts) < 2:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid purchase order ID format"
            )
        ...
        if verification.get("status") == "Completed":
            ...
            logger.warning(
                f"Payment {payment.id} is PENDING. " f"Manual verification may be required."
            )
            ...
            logger.info(
                f"Payment {payment.id} marked as FAILED. " f"Reason: {verification.get('status')}"
            )
...
async def esewa_callback(
    session: Annotated[AsyncSession, Depends(get_async_db)],
    oid: str | None = Query(None, description="Order ID / Transaction ID"),
    amt: float | None = Query(None, description="Amount"),
    refId: str | None = Query(None, description="eSewa reference ID"),
) -> dict:
    """
...
async def imepay_callback(
    session: Annotated[AsyncSession, Depends(get_async_db)],
    TransactionId: str | None = Query(None, description="IME Pay transaction ID"),
    RefId: str | None = Query(None, description="Merchant reference ID"),
    Msisdn: str | None = Query(None, description="Customer mobile number"),
    Amount: float | None = Query(None, description="Transaction amount"),
    Status: str | None = Query(None, description="Payment status"),
) -> dict:
    """
