...
from ...schemas.message import (BulkMessageCreate, BulkMessageResponse,
                                MessageResponse, MessageSchedule,
                                MessageStatistics)
...
async def send_bulk_messages(
    ...
    if len(tenants) != len(bulk_request.tenant_ids):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="One or more tenants not found"
        )
    ...
    for tenant in tenants:
        try:
            ...
            message = Message(
                ...
                template=bulk_request.template,
                subject=(
                    message_service._get_email_subject(bulk_request.template, variables)
                    if bulk_request.channel == MessageChannel.EMAIL
                    else None
                ),
                content=content,
                channel=bulk_request.channel,
                ...
                status=(
                    MessageStatus.SCHEDULED if bulk_request.scheduled_at else MessageStatus.PENDING
                ),
...
async def list_messages(
    ...
    channel: Annotated[MessageChannel | None, Query(description="Filter by channel")] = None,
    status_filter: Annotated[
        MessageStatus | None, Query(alias="status", description="Filter by status")
    ] = None,
    bulk_message_id: Annotated[str | None, Query(description="Filter by bulk message ID")] = None,
...
async def get_message_statistics(
    current_user: Annotated[User, Depends(get_current_user)],
    session: Annotated[AsyncSession, Depends(get_db)],
    start_date: Annotated[datetime | None, Query(description="Start date for statistics")] = None,
    end_date: Annotated[datetime | None, Query(description="End date for statistics")] = None,
):
    """
    ...
    query = select(
        ...
        func.count(func.nullif(Message.status == MessageStatus.DELIVERED, False)).label(
            "delivered"
        ),
        func.count(func.nullif(Message.status == MessageStatus.FAILED, False)).label("failed"),
        func.count(
            func.nullif(Message.status.in_([MessageStatus.PENDING, MessageStatus.SCHEDULED]), False)
        ).label("pending"),
...
async def get_message(
    ...
    """
    result = await session.execute(
        select(Message).where(and_(Message.id == message_id, Message.sent_by == current_user.id))
    )
