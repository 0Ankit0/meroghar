...
from ...schemas.bill import (BillAllocationCreateRequest,
                             ...
                             RecurringBillListResponse, RecurringBillResponse,
...
async def list_bills(
    ...
    bill_type: BillType | None = Query(None, description="Filter by bill type"),
    status: BillStatus | None = Query(None, description="Filter by bill status"),
) -> BillListResponse:
...
async def list_recurring_bills(
    ...
    try:
        ...
        return RecurringBillListResponse(
            recurring_bills=[RecurringBillResponse.model_validate(rb) for rb in recurring_bills],
            total=total,
            skip=commons.skip,
            limit=commons.limit,
        )
...
async def update_recurring_bill(
    ...
    try:
        ...
        logger.info(f"Recurring bill updated: id={recurring_bill_id}, user_id={current_user.id}")
...
async def generate_recurring_bills(
    current_user: Annotated[User, Depends(require_role(UserRole.INTERMEDIARY))],
    session: Annotated[AsyncSession, Depends(get_async_db)],
    recurring_bill_id: UUID | None = Query(None, description="Optional specific recurring bill ID"),
) -> BillListResponse:
