...
class DocumentCreate(BaseModel):
    ...
    reminder_days_before: int = Field(30, ge=1, le=365, description="Days before expiration to send reminder")
    tenant_id: Optional[int] = Field(None, description="Related tenant ID")
...
class DocumentComplete(BaseModel):
    ...
    def validate_mime_type(cls, v):
        ...
        if v not in allowed_types:
            raise ValueError(f'MIME type {v} not allowed. Allowed types: {", ".join(allowed_types)}')
        return v
