...
class SyncRecordRequest(BaseModel):
    ...
    client_id: Optional[str] = Field(
        None,
        description="Client-side temporary ID for tracking (useful for CREATE operations)"
    )
    model: str = Field(
        ...,
...
class SyncRequest(BaseModel):
    ...
    records: List[SyncRecordRequest] = Field(
        ...,
        description="List of records to sync"
    )
...
class SyncRecordResponse(BaseModel):
    ...
    client_id: Optional[str] = Field(None, description="Client-side temporary ID")
    status: str = Field(
        ...,
        description="Result status: 'success', 'failed', or 'conflict'"
    )
    message: str = Field(..., description="Human-readable result message")
    server_id: Optional[int] = Field(
        None,
        description="Server-assigned ID (for CREATE operations)"
    )
...
class SyncStatusResponse(BaseModel):
    ...
    device_id: Optional[str] = Field(None, description="Device ID (if filtered)")
    ...
    latest_sync_at: Optional[str] = Field(None, description="Latest sync timestamp (ISO format)")
    latest_sync_status: Optional[str] = Field(None, description="Latest sync status")
...
class ConflictResponse(BaseModel):
    ...
    conflict_details: Optional[Dict[str, Any]] = Field(
        None,
        description="Structured conflict information (models, records, timestamps)"
    )
