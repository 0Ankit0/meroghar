...
class PropertyCreateRequest(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=255, description="Property name/identifier"
    )
    address_line1: str = Field(
        ..., min_length=1, max_length=255, description="Street address"
    )
    address_line2: Optional[str] = Field(
        None, max_length=255, description="Apartment/unit number"
    )
    ...
    def validate_currency_code(cls, v: str) -> str:
        ...
        if v_upper not in valid_codes:
            raise ValueError(
                f"Invalid currency code. Supported: {', '.join(sorted(valid_codes))}"
            )
        return v_upper
...
class PropertyUpdateRequest(BaseModel):
    ...
    state: Optional[str] = Field(
        None, min_length=1, max_length=100, description="State/province"
    )
    ...
    total_units: Optional[int] = Field(
        None, gt=0, description="Number of rental units"
    )
...
class PropertyResponse(BaseModel):
    ...
    address_line2: Optional[str] = Field(None, description="Apartment/unit number")
...
class PropertyAssignmentResponse(BaseModel):
    ...
    removed_at: Optional[datetime] = Field(
        None, description="Removal timestamp (soft delete)"
    )
