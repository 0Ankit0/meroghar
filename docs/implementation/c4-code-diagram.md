...
## Rental Application Component — Code Level
...
classDiagram
    ...
    class BookingService {
        ...
        +create_booking(customer_id, data) Rental Application
        +confirm_booking(booking_id, owner_id) Rental Application
        +cancel_booking(booking_id, actor_id, reason) Rental Application
        +initiate_return(booking_id, actual_return_at) Rental Application
        +modify_booking(booking_id, new_start, new_end) Rental Application
    }
    ...
    class BookingRepository {
        -db: AsyncSession
        +find_by_id(id) Optional[Rental Application]
        +find_by_owner(owner_id, filters) List[Rental Application]
        +find_by_customer(customer_id, filters) List[Rental Application]
        +find_active_past_end_time(current_time) List[Rental Application]
        +save(rental application) Rental Application
        +update_status(booking_id, status) Rental Application
    }
